...
Span gateway exposes it as `/run/devc2/spans/ssh-agent.sock`, and the
Box points `SSH_AUTH_SOCK` there directly. No command, SSH-specific protocol
client, or second agent proxy runs inside the Box. Entrypoint creates one
...
Only the inherited Span listener is validated during startup. The selected
