...
URLs, raw-token operations, and authentication refresh from the Box.
...
devc2 resolves that World through the Span gateway; OpenAI supplies no Box
client code. Stock Tact and Pi receive only the same fake bearer used by the
proxy, never the host token.

The gateway streams declared CONNECT traffic to the World, preserves
ordinary HTTPS routing, and removes all attachment state with the Box. The World independently enforces
...
access token expiring within one hour; refresh tokens never cross the gateway.
