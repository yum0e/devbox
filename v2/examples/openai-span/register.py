#!/usr/bin/env python3
"""Install one immutable OpenAI Span generation and register it atomically."""
...
def install_generation(root: Path, provider: bytes, client: bytes) -> Path:
    ...
    if generation.exists():
        if (generation / "provider").read_bytes() != provider or (generation / "client").read_bytes() != client:
            raise RuntimeError(f"existing OpenAI generation is inconsistent: {generation}")
        return generation
    staging = Path(tempfile.mkdtemp(prefix=".openai.", dir=root))
    try:
...
def main() -> int:
    ...
    with os.fdopen(lock, "a+") as locked:
        ...
        document["spans"]["openai"] = {
            "provider": str(generation / "provider"),
            "client": str(generation / "client"),
        }
        if len(document["spans"]) > 64:
            raise RuntimeError("Span catalog contains too many entries")
        write_catalog(catalog, document)
    print(f"installed immutable OpenAI generation: {generation}")
    print(f"registered OpenAI Span: {catalog}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"openai-span installer: {error}", file=sys.stderr)
        raise SystemExit(1)
