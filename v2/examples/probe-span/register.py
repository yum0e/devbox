#!/usr/bin/env python3
"""Install one immutable probe generation and register it atomically."""
...
def install_generation(root: Path, provider: bytes, client: bytes) -> Path:
    ...
    if generation.exists():
        if (generation/"provider").read_bytes()!=provider or (generation/"client").read_bytes()!=client:
            raise RuntimeError(f"existing probe generation is inconsistent: {generation}")
        return generation
    staging=Path(tempfile.mkdtemp(prefix=".probe.",dir=root))
    try:
        ...
        try: os.replace(staging,generation)
...
def main() -> int:
    ...
    with os.fdopen(lock,"a+") as locked:
        ...
        document["spans"]["probe"]={"provider":str(generation/"provider"),"client":str(generation/"client")}
        if len(document["spans"])>64:
            raise RuntimeError("Span catalog contains too many entries")
        write_catalog(catalog,document)
    print(f"installed immutable probe generation: {generation}")
    print(f"registered probe Span: {catalog}")
    return 0
...
if __name__=="__main__":
    try: raise SystemExit(main())
        ...
        print(f"probe-span installer: {error}",file=sys.stderr)
        raise SystemExit(1)
